...
func TestSetStatus(t *testing.T) {
	...
	for _, status := range parceStatus {
...
func TestGetByClient(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, len(parcelMap), len(storedParcels)) // получите список посылок по идентификатору клиента, сохранённого в переменной client
	// убедитесь в отсутствии ошибки
	// убедитесь, что количество полученных посылок совпадает с количеством добавленных
	// check
	for _, parcel := range storedParcels {
		assert.NotEqual(t, 0, parcel.Number)
		assert.NotEqual(t, 0, parcel.Client)
